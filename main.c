    ...
    for (int i = 0; i < strlen(input) - 1; i++) { // - 1 for the \n at the end
        ...
        if (isNum(character) && isOperator(character)) { // + or -
            if (i == 0 || tokens[tokensLen - 1].type == 0) {
                ...
                currType = 1;
                currTok[strlen(currTok)] = character; // set the character
            } else {
                currType = 0;
    ...
    if (head == -1) {
        if (size == 1) {
            ...
            if (tokens[0].type == 2 && tokens[size - 1].type == 2) { // there are brackets surrounding the tokens
                return findHead(&(tokens[1]), size - 2); // pointer stuff to get the array starting at the 2nd item, since size is 2 less it will ignore the item at the end
            } else {
...
struct Node parse(struct Token tokens[], int size) {
    ...
    if (size != 1) {
        struct Node left = parse(tokens, headIndex);
        lefts[leftIndex] = left;
        head.left = &lefts[leftIndex];
        leftIndex++;
        
        struct Node right = parse(&(tokens[headIndex + 1]), size - headIndex - 1);
